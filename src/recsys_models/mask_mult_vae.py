...
from src.nn_modules.mask import Mask
...
class MaskMultVAE(AdvBaseRecModel, VaeRecModel):
    def __init__(
        self,
        encoder_dims,
        decoder_dims=None,
        ...
        self.mask = Mask(encoder_dims, mask_hidden_dim)

        #self.pretrained = pretrained #None or path to pretrained model
        if pretrained is None:
            self.pretrained = None
        else:
            self.pretrained = os.path.join(pretrained, str(fold), "train/best_model_utility.pt")
        #####
        ...
        if self.pretrained is not None:
            self.load_state_dict(torch.load(self.pretrained, weights_only=True))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.use_mask:
            m, mh = self.mask(x)
            z, mu, logvar = self.encoder_forward(m)
        else:
        ...
        return z, mu, logvar, m, adv_results
    ...
    def mask_user(self, x):
        return self.mask(x)

    def set_mask(self, bool=True):
        self.use_mask = bool
    ...
    def calc_adv_losses(self, adv_pred, adv_targets):
        ...
        for logits, targets, (feature, adv_module_loss) in zip(
            ...
            module_adv_loss = module_adv_loss / len(logits)
            module_key = f"adv_loss/{feature}_{adv_module_loss.__class__.__name__}"
            adv_losses_dict[module_key] = module_adv_loss
            adv_loss += module_adv_loss
    ...
    @staticmethod
    def build_from_config(config: dict, dataset: FairDataset):
        model_config = config["model_config"]
        fold = config["dataset_config"]["fold"]
        ...
        #use_mask = model_config.get("use_mask")
        #mask_hidden_dim = model_config.get("mask_hidden_dim")
        #adv_warmup = model_config.get("adv_warmup")
        ...
        return MaskMultVAE(encoder_dims, decoder_dims, fold=fold, **model_config)
