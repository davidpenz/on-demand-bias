...
from src.data.data_loading import get_dataset_and_dataloader
...
def generate_job_configuration(
    ...
    verbose = max_jobs == 1
    folds = list(range(input_config.get("n_folds", 1)))
    for rd in results_dirs:
        atk_results_dir = rd["results_dir"]
        conf = deepcopy(attacker_config)
        # conf.update(yaml_load(input_config[""]))
        conf.update(
            {
        ...
        # TODO: Add logic to support any type of input

        embedding_size = conf["rec_model_config"]["model_config"]["latent_size"]
        conf["model_config"].update({"embedding_size": embedding_size})

        conf["trainer"]["dataloader"].update({"n_workers": input_config["n_workers"]})
        conf["verbose"] = verbose
        # Filtering not indicate folds
        run_fold = conf["rec_model_config"]["dataset_config"]["fold"]
        conf["dataset_config"].update(conf["rec_model_config"]["dataset_config"])

        if run_fold in folds:
...
def run_trainrec_attack(config: dict):
    reproducible(config["random_state"])
    splits = ["train", "val", "test"]
    config["dataset_config"]["splits"] = splits
    datasets, dataloaders = get_dataset_and_dataloader(config)
    ...
    rec_model_conf = config.get("rec_model_config", None)
    rec_model = get_model(rec_model_conf, train_dataset).to(config["device"])
    atk_model = get_attacker(config, train_dataset).to(config["device"])
