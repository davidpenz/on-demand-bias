...
class ResamplingStrategy(
    str, Enum
):  # inherit from str for serialization, see https://stackoverflow.com/a/51976841
    NoResampling = "none"
...
def resample_dataset(
    ...
    if len(features) != 1 and strategy != ResamplingStrategy.NoResampling:
        raise AttributeError(
            "For experiments with multiple features, dataset resampling is not available."
        )
    ...
    if strategy == ResamplingStrategy.NoResampling:
        ...
        sampler_cls = (
            RandomUnderSampler
            if strategy == ResamplingStrategy.Undersample
            else RandomOverSampler
        )
        sampler = sampler_cls(**strategy_params, random_state=random_state)

        interaction_matrix_sampled, user_info_sampled = sampler.fit_resample(
            X=interaction_matrix, y=user_info[feature]
        )
        ...
        user_info_sampled = user_info_sampled[
            ["userID", features]
...
def perform_kfold_split(
    ...
    for i in range(n_folds):
        ...
        tr_im_os, df_y_os = resample_dataset(
            strategy=resampling_strategy,
            interaction_matrix=tr_im,
            user_info=tr_user_info,
            features=features,
...
def ensure_make_data(
    ...
    target_path: str,
    features: list = None,
    resampling_strategy: ResamplingStrategy = ResamplingStrategy.NoResampling,
    random_state: int = 42,
    drop_fold_items=True,
):
    """
